...
<table id="table-1">
	<tr><td>
		 <h3>All Deposits</h3>
		 <h4><a href="<%=request.getContextPath()%>/back_end/dep/Depositcheck_back.jsp"><button type="button" class="btn btn-light">回首頁</button></a></h4>
	</td></tr>
</table>
...
<table>
	...
	<c:forEach var="depVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
      ...
      <td>${depVO.dep_day}</td>
      <c:if test="${depVO.dep_sta == 0 }">
      <td>未申請</td>
      </c:if>
      <c:if test="${depVO.dep_sta == 1 }">
      <td>    
    <input type="hidden" name="action" value="apply_DepCoa_back">							
	<input type="submit" class="t1 is"  id="sta${depVO.dep_sta}" value="${depVO.dep_sta==1?"申請中(待確認)":"申請完成"}" depid="${depVO.dep_id}">
	</td>
      </c:if>
    </tr>
    </c:if>
