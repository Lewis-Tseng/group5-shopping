...
<ul>
  <li><a href='<%=request.getContextPath()%>/back_end/dep/ListallMemDep_back.jsp'>List</a> All MembersDep  <br><br></li>
  
  <li><a href='<%=request.getContextPath()%>/back_end/dep/ListallCoaDep_back.jsp'>List</a> All CoachsDep  <br><br></li>
  ...
  <li>
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front_end/dep/dep.do" >
       <b>選擇教練編號:</b>
       <select size="1" name="dep_id">
       
         <c:forEach var="depVO" items="${depSvc.all}" >    
	            <c:if test="${depVO.coa_id!=null}">
	          				<option value="${depVO.dep_id}">${depVO.coa_id}    
	             </c:if>
         </c:forEach>   
         
       </select>
       <input type="hidden" name="action" value="getOne_For_CoaDepDisplay">
       <input type="submit" value="送出">
    </FORM>
  </li>
  ...
  <li>
     <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front_end/dep/dep.do" >
       ...
       <select size="1" name="dep_id">
         <c:forEach var="depVO" items="${depSvc.all}" > 
	         <c:if test="${depVO.coa_id==null}">
	          <option value="${depVO.dep_id}">${depVO.mem_id}
	          </c:if>
         </c:forEach>   
       </select>
       <input type="hidden" name="action" value="getOne_For_MemDepDisplay">
       <input type="submit" value="送出">
     </FORM>
  </li>
</ul>
