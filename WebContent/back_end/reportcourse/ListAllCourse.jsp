...
<%@page import="com.reportcrs.model.CourseReportVO"%>
<%@page import="com.reportcrs.model.CourseReportService"%>
<%@page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@page import="java.util.*"%>
<%@page import="java.io.*"%>
<%
	CourseReportService reportcrsSvc =new CourseReportService();
	List<CourseReportVO> list =reportcrsSvc.getAll();
...
<!--  	********************************************************************************************************** -->
				...
				<div class="container-fluid">
					...
					<c:if test="${not empty errorMsgs}">
					 ...
					 <h4><a href="SelectReportCourse.jsp">回到檢舉課程管理</a></h4>
					<table class="newstable">
						...
						<c:forEach var="reportcrsVO" items="${list}" >
							<tr>
								<td>${reportcrsVO.crs_no}</td>
								<td>${crsSvc.getOneCrs(reportcrsVO.crs_no).crs_name}</td>
								<td>${reportcrsVO.rep_content}</td>
								<td>
								<c:if test="${reportcrsVO.rep_status == '1'}">已處理</c:if>
								<c:if test="${reportcrsVO.rep_status == '0'}">未處理</c:if>
			  					 </td>
								<td>
									<FORM METHOD="post"ACTION="<%=request.getContextPath()%>/back_end/reportcourse/reportcrs.do"
									style="margin-bottom: 0px ">
										<input type="submit" value="修改狀態">
										<input type="hidden"name="rep_id" value="${reportcrsVO.crs_repid}"> 
										<input type="hidden" name="action" value="ChangeStatus">
									</FORM>
								</td>
								<td>
									<FORM METHOD="post"
										ACTION="<%=request.getContextPath()%>/back_end/reportcourse/reportcrs.do"
										style="margin-bottom: 0px;">
										<input type="submit" value="刪除"> <input type="hidden"
											name="rep_id" value="${reportcrsVO.crs_repid}"> 
