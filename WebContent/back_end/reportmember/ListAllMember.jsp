...
<%
	...
	ReportMemService reportmemSvc = new ReportMemService();
	List<ReportMemVO> list =  reportmemSvc.getAll();
...
<!--  	********************************************************************************************************** -->
				...
				<div class="container-fluid">
					...
					<c:if test="${not empty errorMsgs}">
					 ...
					 <h4><a href="SelectReportMember.jsp">回到檢舉會員管理</a></h4>
					<table class="newstable">
						...
						<c:forEach var="reportmemVO" items="${list}" >
							<tr>
								<td>${reportmemVO.rep_id}</td>
								<td>${reportmemVO.mem_id}</td>
								<td>
							
	                    			${memSvc.getOneMem(reportmemVO.mem_id).mem_name} 
								</td>
								<td>${reportmemVO.rep_cot}</td>
								<td>${reportmemVO.rep_sta}</td>
								<td>
									<FORM METHOD="post"ACTION="<%=request.getContextPath()%>/back_end/reportmember/reportmem.do"
									style="margin-bottom: 0px ">
										<input type="submit" value="修改">
										<input type="hidden"name="rep_id" value="${reportmemVO.rep_id}"> 
										<input type="hidden" name="action" value="getOne_For_Update">
									</FORM>
								</td>
								<td>
									<FORM METHOD="post"
										ACTION="<%=request.getContextPath()%>/back_end/reportmember/reportmem.do"
										style="margin-bottom: 0px;">
