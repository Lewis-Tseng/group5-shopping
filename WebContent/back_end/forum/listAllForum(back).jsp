...
<html>
<head>
<title>文章討論區後臺管理 - listAllForum.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>所有文章 - listAllForum.jsp</h3>
		 <h4><a href="<%=request.getContextPath()%>/forum/listAllForum.jsp"><img src="images/back1.gif" width="100" height="32" border="0">回首頁</a></h4>
	</td></tr>
</table>
...
<table>
	...
	<c:forEach var="forumVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr>
			...
			<td><c:if test="${forumVO.forum_stat == '1'}">上架中</c:if>
				<c:if test="${forumVO.forum_stat == '0'}">下架</c:if>
			    </td>
 			<td>
