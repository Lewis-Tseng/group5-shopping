...
<link
	href="<%=request.getContextPath()%>/back_end/vendor/fontawesome-free/css/all.css"
...
<link href="<%=request.getContextPath()%>/back_end/css/sb-admin-2.css"
	rel="stylesheet">
...
table {
	...
	color: #ffe153;
	font-size: 25px;
	background-color: rgba(100, 156, 249, 0.49);
	 border-radius: 40px;
		...
		<ul
			...
			<a
				...
				<div class="sidebar-brand-icon rotate-n-15">
					<i class="fas fa-laugh-wink"> <img
						src="<%=request.getContextPath()%>/back_end/img/yt.png">
					</i>
				</div>
...
<jsp:useBean id="emp_authSvc" scope="page" class="com.emp_auth.model.EaService"/>
<jsp:useBean id="authSvc" scope="page" class="com.authority.model.AuthService"/>
<FORM METHOD="post" ACTION="adm.do" id="form1" enctype="multipart/form-data">
<h1>新增員工:</h1>
<h4><a href="SelectEmp.jsp">回到員工查詢</a></h4>
<%-- 錯誤表列 --%>
...
<table>
 	<tr>
		<td>員工姓名:</td>
		<td><input type="TEXT" name="emp_name" size="45" value="<%= (admVO==null)? "" : admVO.getEmp_name()%>" /></td>
	</tr>
	<tr>
		<td>員工信箱:</td>
		<td><input type="email" name="emp_email" title="請輸入mail" size="45"
		value="<%= (admVO==null)? "@gmail.com":admVO.getEmp_email() %>"
		></td>
	</tr>
	<tr>
	<td>員工照片</td>
	<td><img id="output0" style="widith:30%;height:30%"/><br>
	<span><input type="file" name="file" onchange="loadFile(event)" value="<%=(admVO == null) ? "" : admVO.getEmp_pic()%>"/></span>
	</td>
	</tr>
	<tr>
		<td>員工權限:</td>
		<td>
			<c:forEach var="authVO" items="${authSvc.all}">
				<input type="checkbox" name="authid" value="${authVO.auth_id}" ${(admVO.emp_id==authVO.auth_id)? 'selected':'' } >${authVO.auth_name}
			</c:forEach>
		</td>
	</tr>
</table>
...
<!--************************************** Logout 開始 ********************************************************************************************************* -->
			...
			<script
				src="<%=request.getContextPath()%>/back_end/js/sb-admin-2.min.js"></script>

			<!-- Page level plugins -->
			<script
				src="<%=request.getContextPath()%>/back_end/vendor/chart.js/Chart.min.js"></script>

			<!-- Page level custom scripts -->
			<script
				src="<%=request.getContextPath()%>/back_end/js/demo/chart-area-demo.js"></script>
			<script
				src="<%=request.getContextPath()%>/back_end/js/demo/chart-pie-demo.js"></script>
</body>
