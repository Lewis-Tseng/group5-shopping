...
<!--  	********************************************************************************************************** -->
				...
				<%-- 錯誤表列 --%>
					<c:if test="${not empty errorMsgs}">
					 ...
					 <h1 style="text-align:center"><a href="<%=request.getContextPath()%>/back_end/adm/SelectEmp.jsp">回到員工查詢</a></h1>
			...
			<tr>
				<td><span><img style="width:150px;heigh:150px" src="<%=request.getContextPath()%>/EmpReaderPicServlet?emp_id=${admVO.emp_id} "></span></td>
				<td>${admVO.emp_id}</td>
				<td>${admVO.emp_name}</td>
			<td><c:forEach var="eaVO" items="${eaSvc.all}">
                    <c:if test="${admVO.emp_id==eaVO.emp_id}">
					${authSvc.getOneAuth(eaVO.auth_id).auth_name}<br>
                    </c:if>
                </c:forEach>
			</td>
		</tr>
