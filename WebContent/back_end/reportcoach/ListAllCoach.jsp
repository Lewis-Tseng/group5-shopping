...
<%@page import="com.reportcoa.model.ReportCoaService"%>
...
<%
	ReportCoaService reportcoaSvc = new ReportCoaService();
	List<ReportCoaVO> list = reportcoaSvc.getAll();
...
<!--  	********************************************************************************************************** -->
				...
				<div class="container-fluid">
					...
					<c:if test="${not empty errorMsgs}">
					 ...
					 <h4><a href="SelectReportCoach.jsp">回到檢舉教練管理</a></h4>
					<table class="newstable">
						...
						<c:forEach var="reportcoaVO" items="${list}" varStatus="i" >
							<tr>
								<td> ${reportcoaVO.rep_id}</td>
								<td> ${reportcoaVO.coa_id}</td>
								<td>
	                    			${coaSvc.getOneCoa(reportcoaVO.coa_id).coa_name} 
								</td>
								<td>${reportcoaVO.rep_cot}</td>
								
								<td>
								<c:if test="${reportcoaVO.rep_sta == '1'}">已處理</c:if>
								<c:if test="${reportcoaVO.rep_sta == '0'}">未處理</c:if>
			  					 </td>
								<td>
								<FORM METHOD="post"ACTION="<%=request.getContextPath()%>/back_end/reportcoach/reportcoa.do"
									style="margin-bottom: 0px ">
										<input type="submit" value="送出警告">
										<input type="hidden"name="rep_id" value="${reportcoaVO.rep_id}"> 
										<input type="hidden"name="coach_id" value="${reportcoaVO.coa_id}"> 
										<input type="hidden" name="action" value="ChangeStatus">
									</FORM>
							 	</td>
								<td>
									<FORM METHOD="post"
										ACTION="<%=request.getContextPath()%>/back_end/reportcoach/reportcoa.do"
										style="margin-bottom: 0px;">
										<input type="submit" value="刪除"> <input type="hidden"
											name="rep_id" value="${reportcoaVO.rep_id}"> 
