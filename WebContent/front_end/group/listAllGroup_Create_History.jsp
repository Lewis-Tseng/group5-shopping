...
<%@page import="com.gro.group.groupService"%>
...
MemVO memVO = (MemVO) session.getAttribute("memVO");
	...
	}else if(request.getParameter("mem_id")!=null){
	...
	List<groupVO> list = groupSvc.getGroByMemHistory(mem_id);
...
<!--JS for animate-->
	...
	<style type="text/css">
		...
		.example-2 .data {
			transform: translateY(calc(95px + 4em));
		}
...
<!---banner--->
			...
			<div class="banner wow fadeInDownBig animated animated" data-wow-delay="0.4s">
				<div class="container">
					<h4 style="color: rgb(70,70,70); font-family: test,微軟正黑體;font-size:50px;margin-left: 40%;">我曾經建立的揪團</h4>
				</div>
			</div>
		<!---about-->
								...
								<ul class="lb-album">
									<li class="grid">
										<a href="<%=request.getContextPath()%>/front_end/group/char_select.jsp" >
											<figure class="effect-apollo">
												<div><b style="font-family: test">Mmanagement</b><img src="<%=request.getContextPath()%>/front_end/css_group/images/btn2.png"></div>
												<figcaption></figcaption>
											</figure>
										</a>
									</li>
												...
												<div class="data">
													<div class="contents">
														...
														<FORM METHOD="post" ACTION="group.do">
															...
															<input type="hidden" name="action" value="getOne_For_Display_HCreate"> 
							...
							<div class="clearfix"></div>
								<nav>
									...
									<ul class="pagination pagination-lg" style="margin:3% 23% 0% 50%;">
										<%if (whichPage != 1){ %>
										<li><a href="<%=request.getRequestURI()%>?mem_id=<%=mem_id%>&whichPage=<%=whichPage-1%>" aria-label="Previous"><span aria-hidden="true">«</span></a></li>
										<%}%>
										
										<%for (int i = 1; i <= pageNumber; i++) {%>
											<%if(i==whichPage) { %>
												<li><a href="<%=request.getRequestURI()%>?mem_id=<%=mem_id%>&whichPage=<%=i%>" style="cursor: pointer;pointer-events: none;color:#E74C3C;" aria-label="Previous"><span aria-hidden="true"><%=i%></span></a></li>
											<%} else {%>
												<li><a href="<%=request.getRequestURI()%>?mem_id=<%=mem_id%>&whichPage=<%=i%>" aria-label="Previous"><span aria-hidden="true"><%=i%></span></a></li>
											<%}%>
										<%}%>
										
										<%if (whichPage < pageNumber){ %>
										<li><a href="<%=request.getRequestURI()%>?mem_id=<%=mem_id%>&whichPage=<%=whichPage+1%>" aria-label="Next"><span aria-hidden="true">»</span></a></li>
										<%}%>
						 			 </ul>
						 			<%}%>
						 		</nav>
