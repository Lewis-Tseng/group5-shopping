...
<!-- 								</div>	 -->
	...
	<div class="col-md-8 map-middle wow fadeInRight animated animated" data-wow-delay="0.4s" style="margin-left:-45px;">
	...
	<ul>
		<c:forEach var="message" items="${errorMsgs}">
			<li style="color:red">${message}</li>
						...
						<form action="<%=request.getContextPath()%>/mem.do" method="post" enctype="multipart/form-data" style="margin-left:400px;background-color:lightgrey;
    opacity:0.95;font-size:30px">
							<jsp:useBean id="memSvc" scope="page" class="com.mem.model.MemService" />
							<c:forEach var="memVO" items="${memSvc.all}">
			
				            <c:if test="${memVO.mem_email == loginVO.mem_email}">
				            <b>會員姓名</b><br>
							<input type="text" name="mem_name" class="mem_name"  required="" value="${memVO.mem_name}"><br>
							<b>生日</b><br>
							<input type="date" name="birthday" value="${memVO.birthday}"><br><br>

							<b>電話</b><br>
							<input type="text" name="phone" class="tel"   value="${memVO.phone}"><br><br>
							<b>郵遞區號</b><br>
							<input type="text" name="pos_code" class="pos_code"   value="${memVO.pos_code}"><br><br>

							<b>地址</b>
							<input type="text" name="address" class="address"   value="${memVO.address}">

							<b>大頭照</b>
							
<%-- 							<img   src="<c:if test="${memVO.mem_id != null}"><%=request.getContextPath()%>/DBGifReaderMem?mem_id=${memVO.mem_id}</c:if> --%>
<%-- 							<c:if test="${memVO.mem_id == null}"><%=request.getContextPath()%>/front_end/mem/images/G5_LOGO_v2.0.png</c:if>"/> --%>
							
							<input name="mem_pic" type="file" onchange="readURL(this)" target="<%=request.getContextPath()%>/DBGifReaderMem?mem_id=${memVO.mem_id}" accept="image/gif, image/jpeg, image/png"/ >
							<img style="width:150px;height:150px;" id="<%=request.getContextPath()%>/DBGifReaderMem?mem_id=${memVO.mem_id}" src="
							<c:if test="${memVO.mem_id != null}"><%=request.getContextPath()%>/DBGifReaderMem?mem_id=${memVO.mem_id}</c:if>
							<c:if test="${memVO.mem_id == null}"><%=request.getContextPath()%>/front_end/mem/images/G5_LOGO_v2.0.png</c:if>" />
							<br><br>
							<input type="hidden" name="action" value="update_mem_personal">
